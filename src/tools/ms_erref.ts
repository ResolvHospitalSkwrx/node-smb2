...
const severities: { [key: number]: ErrorInfo } = {
  0:{
...
const facilities: { [key: number]: ErrorInfo } = {
  0:{
    "code":"FACILITY_NULL"
...
const ntStatus: { [key: string]: ErrorInfo } = {
  "0x00000000":{
    "code":"STATUS_SUCCESS"
...
const win32ErrorCodes: { [key: string]: ErrorInfo } = {
  "0x00000000":{
    "code":"ERROR_SUCCESS"
  , "message":"The operation completed successfully."
  }
...
export function getStatus(errorCode: number): StatusError {
  const hex = '0x' + ('00000000' + errorCode.toString(16)).toUpperCase().substr(-8);
  const err: any = ntStatus[hex] || win32ErrorCodes[hex] || {
  ...
  const facility = (errorCode >> 16) & 0x7ff;
  ...
  const severity = (errorCode >> 30) & 0x03;
...
export function getErrorMessage(err: StatusError): string {
