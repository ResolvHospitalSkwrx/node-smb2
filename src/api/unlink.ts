...
export default function unlink(this: any, path: string, cb: SMB2Callback): void {
  const connection = this;

  connection.exists(path, (err: Error | null, exists: boolean) => {
    if (err) cb && cb(err);
    else if (exists) {
      SMB2Request('create', { path }, connection, (err, file) => {
        if (err) cb && cb(err);
        else
          SMB2Request(
            'set_info',
      ...
      cb && cb(new Error('File does not exists'));
    }
  });
}
