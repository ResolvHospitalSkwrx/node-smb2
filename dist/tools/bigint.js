...
class BigInt {
        ...
        if (BigInt.isBigInt(n)) {
            ...
            if (v !== 0) {
                ...
                for (let i = 0; i < size; i++) {
                    ...
                    this.buffer.writeUInt8(parseInt(start === -1 ? hexValue.substr(0, 1) : hexValue.substr(start, 2), 16), i);
                }
            }
        }
    }
    ...
    add(v) {
        ...
        if (!BigInt.isBigInt(v)) {
            vBigInt = BigInt.toBigInt(this.buffer.length, v);
        }
        else {
            vBigInt = v;
        }
        ...
        for (let i = 0; i < n; i++) {
            const r = (i < this.buffer.length ? this.buffer.readUInt8(i) : 0) +
